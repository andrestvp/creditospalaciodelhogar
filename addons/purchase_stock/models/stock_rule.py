...
class StockRule(models.Model):
    ...
    def _get_lead_days(self, product):
        ...
        if supplier_delay:
            delay_description += '<tr><td>%s</td><td class="text-right">+ %d %s</td></tr>' % (_('Plazo de entrega del proveedor'), supplier_delay, _('day(s)'))
        security_delay = buy_rule.picking_type_id.company_id.po_lead
        delay_description += '<tr><td>%s</td><td class="text-right">+ %d %s</td></tr>' % (_('Tiempo de Compra de Clientes Potenciales de Seguridad'), security_delay, _('day(s)'))
        days_to_purchase = buy_rule.company_id.days_to_purchase
        delay_description += '<tr><td>%s</td><td class="text-right">+ %d %s</td></tr>' % (_('Días para comprar'), days_to_purchase, _('day(s)'))
