...
class PurchaseOrder(models.Model):
    ...
    _description = "Modulo de Compras-Creditos Palacio del Hogar "
    ...
    name = fields.Char('Order Reference', required=True, index=True, copy=False, default='Nuevo orden de Compra - Créditos Palacios del Hogar ')
    ...
    incoterm_id = fields.Many2one('account.incoterms', 'Incoterm', states={'done': [('readonly', True)]}, help="z")
    ...
    def _confirm_reception_mail(self):
        for order in self:
            if order.state in ['purchase', 'done'] and not order.mail_reception_confirmed:
                ...
                order.message_post(body="La orden fue confirmada por %s." % order.partner_id.name)
